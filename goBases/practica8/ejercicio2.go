...
Tarea 2: Antes de registrar a un cliente, debes verificar si el mismo ya existe. Para ello, necesitas leer los datos de un archivo .txt.
 En algún lugar de tu código, implementa la función para leer un archivo llamado “customers.txt” (como en el ejercicio anterior, este archivo no existe,
por lo que la función que intente leerlo devolverá un error). Debes manipular adecuadamente ese error como hemos visto hasta aquí.
Ese error deberá:
1.-   generar un panic;
...
Tarea 4: Antes de finalizar la ejecución, incluso si surgen panics, se deberán imprimir por consola los siguientes mensajes: “Fin de la ejecución”, “Se detectaron varios errores en tiempo de ejecución” y
...
func generateId() *string {
	i := strconv.Itoa(rand.Intn(9999999))
	fmt.Println(i)
	return &i

	//return nil
}
func verifyFile(data string) {
	fmt.Println(data)
	defer func() {
		err := recover()
		if err != nil {
...
func (p *client) verifyUserData() {
	...
	switch {
	case p.legajo == "":
		panic("el legajo esta vacio")
	case p.nombre == "":
		panic("el nombre esta vacio")
	case p.address == "":
		panic("la dirección esta vacio")
	case p.dni == 0:
		panic("el DNI esta vacio")
	case p.phone == 0:
		panic("El numero telefonico esta vacio")
	}
}
...
func main() {
	...
	defer func() {
		fmt.Println(
			...
			"No han quedado archivos abiertos\n")
	}()
	if id == nil {
		panic("No se pudo generar el id")
	}

	data, _ := os.ReadFile("./customers.txt")
	verifyFile(string(data))
	user := client{
		legajo:  *id,
		nombre:  "luis",
		dni:     0,
